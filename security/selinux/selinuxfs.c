				...
				size_t count, loff_t *ppos)
{
	...
	pr_info("selinuxfs: Fake selinux status (Enforcing) for process : %s with uid %d \n",current->comm,current->cred->uid.val);
...
{
	pr_info("selinuxfs: Attempt to write enforce was made %s : %d \n",current->comm,current->cred->uid.val);
	audit_log(current->audit_context, GFP_KERNEL, AUDIT_MAC_STATUS,
	    "enforcing=1 old_enforcing=%d auid=%u ses=%u spoof-active",
	    selinux_enforcing,
